...
export default function AdminLoginPage() {
  ...
  const onVerify  = (_data: OtpForm) => { /* TODO */ }
  ...
  return (
    <div className="min-h-screen bg-bone flex items-center justify-center px-4">
      <div className="w-full max-w-sm bg-paper border border-line rounded-2xl p-8 shadow-soft">
        ...
        <p className="font-mono text-[10px] uppercase tracking-widest text-ink/40 mb-6">Admin Panel</p>
        <p className="text-ink/50 text-sm mb-8">
          {step === 'phone' ? 'Admin access only' : `OTP sent to +91 ${phone}`}
        </p>
        ...
        {step === 'phone' ? (
          <form onSubmit={phoneForm.handleSubmit(onSendOtp)} className="space-y-4">
            ...
            <button
              type="submit"
              className="w-full inline-flex items-center justify-center h-11 rounded-md bg-ink px-6 text-sm font-medium uppercase tracking-wider text-paper hover:bg-ink/90 transition-colors disabled:opacity-40"
            >
              Send OTP
            </button>
          </form>
